...
export default function CourseDetails() {
    ...
    // FIX: Using Array.find() directly is more standard unless getCourseById 
    // is a dedicated async function in your hook.
    ...
    const handleEnroll = async () => {
        if (!isAuthenticated) {
            toast({
                ...
                description: 'Please log in to enroll in a course.',
            });
        ...
        try {
            const enrollResult = await enrollCourse(id); 

            if (enrollResult && enrollResult.success) {
                toast({
                    title: 'Enrollment Successful!',
                    description: `You've been enrolled in ${course.title}. Proceeding to payment.`,
                });
                navigate('/payment', { state: { courseId: id, coursePrice: course.price } });
            } else {
                 toast({
                    title: 'Enrollment Failed',
                    description: enrollResult?.message || 'Could not enroll in the course. Please try again.',
                    variant: 'destructive',
                });
            }
        } catch (error) {
